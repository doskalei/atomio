 ...
 */
function CluusMap(element, opt_opts, opt_markers) {
  var dis = this;
  var map =  element;
  ...
  this.resetViewport = function () {
    ...
    for (var i = 0; i < clusters.length; ++i) {
      var cluusCluster = clusters[i];
      var oldZoom = cluusCluster.getCurrentZoom();
      if (oldZoom === null) {
        continue;
      }
      ...
      if (curZoom !== oldZoom) {
        ...
        // If the cluusCluster zoom level changed then destroy the cluusCluster
        // and collect its markers.
        var mks = cluusCluster.getMarkers();
        ...
        cluusCluster.clearMarkers();
        removed++;
        for (j = 0; j < clusters_.length; ++j) {
          if (cluusCluster === clusters_[j]) {
    ...
    // Add the markers collected into marker cluusCluster to reset
 ...
 * @constructor
 * @private
 * @param {cluusMap} cluusMap The marker cluster object
 */
function CluusCluster(cluusMap) {
  var dis = this;
  ...
  var markers_ = [];
  var cluusMap_ = cluusMap;
  var map = cluusMap.getmap();
  
  var zoom_ = map.getZoom();
  
	
  this.createClusterMarker = function(latlng, text, padding){
    var  marker = new L.CircleMarkerEx(latlng, { color: '#0F9900', fillColor: '#0F9900', fillOpacity: 0.4, radius: 20, label: text });
  ...
  this.isInBounds = function (bounds) {
    ...
    var inViewport = true;
    var gridSize = cluusMap_.getGridSize_();
  ...
  this.redraw = function (isForce) {
    ...
    if (zoom_ >= mz || this.getTotalMarkers() === 1) {
      ...
      if (clusterMarker === null) {
        clusterMarker = this.createClusterMarker(center_, this.getTotalMarkers(), cluusMap_.getGridSize_());
...
function CluusMarker(latlng, newIcon){
	this.map_ = null;
	this.latlng_ = latlng;
	this.isHidden_ = false;
//	alert(newIcon.iconUrl);
	this.marker_ = new L.Marker(latlng, {icon: newIcon});
//	this.marker_.setIcon(newIcon);
//CLUUS
	this.remove = function () {
		if (null != this.map_){
			if( this.marker_.mClass!="locate" ){
